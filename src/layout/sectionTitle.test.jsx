...
describe('SectionTitle without action', function() {
	...
	it('should not render a second flex item if there is no action', function() {
		const flexItems = TestUtils.scryRenderedDOMComponentsWithClass(component, 'flex-item');
...
describe('SectionTitle with action', function() {
	...
	beforeEach(() => {
		component = TestUtils.renderIntoDocument(<SectionTitle title={title} action={action} />);
	});
	afterEach(() => {
		component = null;
	});
	...
	it('renders a shrink FlexItem when an action is passed in', function() {
		const flexItems = TestUtils.scryRenderedDOMComponentsWithClass(component, 'flex-item');
		const shrinkFlexItems = TestUtils.scryRenderedDOMComponentsWithClass(component, 'flex-item--shrink');
